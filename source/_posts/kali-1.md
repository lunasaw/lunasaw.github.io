...
1. 在光驱中插入Kali Linux Live DVD来开始。你会看到它的启动菜单。选择`Graphical install`（图形化安装）。
   ...
   ![/img/kaili](https://tva1.sinaimg.cn/large/008i3skNly1gvn60tayfvj60hs0dd0t702.jpg)

2. 选择语言。这里我们选择`English`（英语）。

   ![/img/kaili](https://tva1.sinaimg.cn/large/008i3skNly1gvn613cg79j60m70gu75802.jpg)

3. 选择你的位置。这里我们选择`United States`（美国）。

   ![/img/kaili](https://tva1.sinaimg.cn/large/008i3skNly1gvn6167oz8j60m30gjdgs02.jpg)

4. 选择你的键盘配置。这里我们选择`American English`（美国英语）。

   ![/img/kaili](https://tva1.sinaimg.cn/large/008i3skNly1gvn61ag0afj60m10get9e02.jpg)

5. 下面要完成网络服务配置。输入主机名称，这里我们输入`Kali`。

   ![/img/kaili](https://tva1.sinaimg.cn/large/008i3skNly1gvn61d6nngj60m70a5dgf02.jpg)

6. 下面，我们需要输入域名。这里我们输入`kali.secureworks. com`。

   ![/img/kaili](https://tva1.sinaimg.cn/large/008i3skNly1gvn61gx8opj60m50a0wf202.jpg)

7. 现在你会看到输入root密码的地方，需要输入两次。

   ![/img/kaili](https://tva1.sinaimg.cn/large/008i3skNly1gvn61k783rj60m70gfmyk02.jpg)

8. 选择你的时区，这里我们选择`Eastern`（东方）。

   ![/img/kaili](https://tva1.sinaimg.cn/large/008i3skNly1gvn61nxxmqj60ma0dfgmb02.jpg)

9. 我们现在可以选择磁盘分区方式。你会看到四个选项。选择`Guided - use entire disk`，这会便于你分区。

   ![/img/kaili](https://tva1.sinaimg.cn/large/008i3skNly1gvn61rb0v8j60m50bo3zf02.jpg)

10. 在这一步，你需要知道你的磁盘会被抹掉，点击`Continue`（继续）。

    ![/img/kaili](https://tva1.sinaimg.cn/large/008i3skNly1gvn61vybyaj60m4091wf002.jpg)
...
11. 下面，你有机会选择三个分区方式之一：所有文件放在一个分区、分离`/home`、以及分离`/home/user/var`和`/tmp`。考虑到Kali用于渗透测试，分区不需要也不必要（即使这对于你的桌面主操作系统是个好主意）。这里我们选择`All files in one partition`（所有文件放在一个分区）并点击`Continue`（继续）。
    ...
    ![/img/kaili](https://tva1.sinaimg.cn/large/008i3skNly1gvn6207cylj60m50bhdgi02.jpg)

12. 一旦你看到了一个界面，让你知道将要对你磁盘执行的改动，选择`Yes`之后点击`Continue`（继续）。要注意这是撤销抹掉你磁盘所有数据的最后机会。

    ![/img/kaili](https://tva1.sinaimg.cn/large/008i3skNly1gvn625s4kaj60m30c474v02.jpg)

13. 下面，你会被询问是否希望链接到网络镜像。网络镜像允许你接收到Kali的更新。这里我们选择`Yes`之后点击`Continue`（继续）。

    ![/img/kaili](https://tva1.sinaimg.cn/large/008i3skNly1gvn62e6xb4j60m40aj74o02.jpg)

14. 你可以通过点击`Continue`（继续）跳过HTTP代理界面。

    ![/img/kaili](https://tva1.sinaimg.cn/large/008i3skNly1gvn62mn1yuj60m20a7gm502.jpg)

15. 最后，你会被询问来安装GRUB启动器到主引导记录（MBR）中。选择`Yes`之后点击`Continue`（继续）。

    ![/img/kaili](https://tva1.sinaimg.cn/large/008i3skNly1gvn63464syj60m30b775402.jpg)

16. 祝贺你现在完成了Kali Linux的安装！点击`Continue`，系统会重启并展示登录界面。

    ![/img/kaili](https://tva1.sinaimg.cn/large/008i3skNly1gvn636u6whj60m2090wex02.jpg)
...
1. 插入格式化且可写入的U盘：
   ...
   ![/img/kaili](https://tva1.sinaimg.cn/large/008i3skNly1gvn63a0ox1j60ek042jrc02.jpg)

2. 启动 Win32 Disk Imager。

3. 点击目录图表，选择Kali Linux DVD ISO镜像的位置：

   ![/img/kaili](https://tva1.sinaimg.cn/large/008i3skNly1gvn652d1nnj60bp0623yl02.jpg)

4. 确保`Space used to preserve files across reboots`（用于在启动中保存文件的空间）设置为4096。

   ![/img/kaili](https://tva1.sinaimg.cn/large/008i3skNly1gvn63dryyuj60et0aywey02.jpg)
...
7. 安装完成之后，我们就可以重启电脑，从新创建的Kali Linux U盘以持久存储器来启动了。
   ...
   ![/img/kaili](https://tva1.sinaimg.cn/large/008i3skNly1gvn63hi1yoj60ef06qaa302.jpg)
...
1. 运行VirtualBox，点击`New`（新建）来启动虚拟机向导：
   ...
   ![/img/kaili](https://tva1.sinaimg.cn/large/008i3skNly1gvn63mq9v1j60lq09bq3h02.jpg)

2. 点击`Next`（下一步）按钮，键入虚拟机的名称，并选择OS类型和版本。这里我们选择Linux类型和Ubuntu（64位）作为版本。点击`Next`按钮来继续：

   ![/img/kaili](https://tva1.sinaimg.cn/large/008i3skNly1gvn63pg9r3j60dj0ao0t702.jpg)

3. 选择分配给虚拟机的基本内存（RAM）的总数。我们打算使用默认值，点击`Next`。

4. 为新的虚拟机创建新的虚拟硬盘，点击`Next`按钮。

   ![/img/kaili](https://tva1.sinaimg.cn/large/008i3skNly1gvn63u818pj60dn0aomxr02.jpg)
...
7. 设置虚拟机磁盘文件类型和大小：
   ...
   ![/img/kaili](https://tva1.sinaimg.cn/large/008i3skNly1gvn63yu0tyj60fu0byaal02.jpg)

8. 检查设置是否正确，之后点击`Create`（创建）按钮来开始虚拟磁盘文件的创建。

9. 我们将会返回前面的向导，带有虚拟机参数的概览。点击`Create`以结束：

   ![/img/kaili](https://tva1.sinaimg.cn/large/008i3skNly1gvn6433szsj60lr0gcgml02.jpg)

10. 新的虚拟机创建之后，我们将要安装Kali Linux。

11. 在VirtualBox的主窗口，高亮Kali Linux，之后点击`Settings`（设置）按钮：

    ![/img/kaili](https://tva1.sinaimg.cn/large/008i3skNly1gvn645ubcwj606808gjre02.jpg)

12. 现在基本的安装步骤就完成了，我们需要让你将下载的ISO文件用于虚拟光盘。这会为你节省烧录物理DVD的时间来完成这个安装。在`Settings`界面中，点击`Storage`（存储器）菜单选项：

    ![/img/kaili](https://tva1.sinaimg.cn/large/008i3skNly1gvn648pbpjj60b90acjrn02.jpg)
...
13. 下一步，在`Storage Tree`（存储器树形图）下面，高亮`Empty`（空）磁盘图标，它在`IDE Controller`（IDE控制器）的下面。这户选择我们的虚拟CD/DVD ROM驱动器。在屏幕的最右边，在 `Attributes`底下，点击光盘图表。在上面弹出的菜单上选择你的`Choose a virtual CD/DVD disc file...`（Kali Linux ISO CD/DVD光盘文件）选项，并找到你的ISO。一旦你完成了这些步骤，点击OK按钮。
    ...
    ![/img/kaili](https://tva1.sinaimg.cn/large/008i3skNly1gvn64al3avj60nq0dhmxw02.jpg)
...
2. 在VMware Workstaion的菜单栏上，访问`VM | Install VMware Tools…`：
   ...
   ![/img/kaili](https://tva1.sinaimg.cn/large/008i3skNly1gvn64ec0qvj608508j74e02.jpg)
...
2. Wicd Network Manager会打开，并带有可用网络的列表：
   ...
   ![/img/kaili](https://tva1.sinaimg.cn/large/008i3skNly1gvn64jevjij60eq072dfw02.jpg)

3. 点击`Properties`（属性）按钮来设定网络细节。完成之后点击OK。

   ![/img/kaili](https://tva1.sinaimg.cn/large/008i3skNly1gvn64lsugmj60bi0gpglu02.jpg)
